  ...
  return (
    <div style={{ width: '100%', height: '100%', background: '#1A1815', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 40, maxWidth: 400, padding: '0 28px' }}>
        <h2 style={{ fontFamily: serif, fontSize: 15, fontWeight: 300, color: 'rgba(232,224,208,0.4)', letterSpacing: '0.12em', ...fadeStyle(stages.header, 4) }}>오늘 만난 사람들</h2>

        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 32 }}>
          {patients.map((p, i) => {
            const patientUnasked = unasked.filter(u => u.patientName === p.name)
            return (
              <div key={p.name} style={{ textAlign: 'center', ...fadeStyle(i < count, 4) }}>
        ...
        {lastScene.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, ...fadeStyle(stages.scene, 4) }}>
        ...
        {overtime.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, ...fadeStyle(stages.overtime, 4) }}>
        ...
        {missedCount > 0 && (
          <p style={{
            ...
            ...fadeStyle(stages.missed, 4),
          }}>오늘 {missedCount}명을 더 볼 수도 있었습니다.</p>
        )}

        {isFinalEpisode && <p style={{ fontFamily: serif, fontSize: 14, fontWeight: 300, color: 'rgba(232,224,208,0.35)', letterSpacing: '0.08em', textAlign: 'center', ...fadeStyle(stages.final, 4) }}>다음 주에 다시 옵니다.</p>}
      </div>
